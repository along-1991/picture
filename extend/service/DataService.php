...
class DataService
{
    ...
    public static function update(&$dbQuery, $where = [])
    {
        ...
        if ($field === 'delete') {
            if (method_exists($db, 'getTableFields') && in_array('is_delete', $db->getTableFields())) {
                if(Db::table($table)->where($where)->where($map)->value('is_delete') == '0'){
                    return Db::table($table)->where($where)->where($map)->update(['is_delete' => '1']) !== false;
                }else{
                    return Db::table($table)->where($where)->where($map)->update(['is_delete' => '0']) !== false;
                }
